...
@Service
public class DocumentCompilationService {

    public byte[] generateDocx(SRSDocument document) throws IOException {
        ...
        for (DocumentSection section : document.getSections()) {
            ...
            if (section.getSectionNumber().equals("B"))
            {
                addSection(docx, section);
            }
           else {
    ...
    private void addTitlePage(XWPFDocument docx, SRSDocument document) {
    ...
    private void addTableOfContents(XWPFDocument docx, SRSDocument document) {
    ...
    private void addSection(XWPFDocument docx, DocumentSection section) {
        // Section heading
        XWPFParagraph heading = docx.createParagraph();
        heading.setStyle("Heading1");
        XWPFRun headingRun = heading.createRun();
        headingRun.setText(section.getSectionNumber() + " " + section.getTitle());
        headingRun.setBold(true);
        headingRun.setFontSize(14);
        headingRun.addBreak();


        // Special handling for Appendix B image
            try {
                // Add Class Diagram
                if (section.getClassDiagramImage() != null) {
                    try (InputStream is = new ByteArrayInputStream(section.getClassDiagramImage())) {
                        XWPFParagraph content = docx.createParagraph();
                        content.setIndentationFirstLine(600); // 0.5 inch indent
                        XWPFRun contentRun = content.createRun();
                        contentRun.setText("Class Diagram:");
                        contentRun.addBreak();
                ...
                // Add State Diagram
                if (section.getStateDiagramImage() != null) {
                    try (InputStream is = new ByteArrayInputStream(section.getStateDiagramImage())) {
                        XWPFParagraph content = docx.createParagraph();
                        content.setIndentationFirstLine(600); // 0.5 inch indent
                        XWPFRun contentRun = content.createRun();
                        contentRun.setText("State Diagram:");
                        contentRun.addBreak();
                        contentRun.addPicture(is,
                                Document.PICTURE_TYPE_PNG,
                                "state_diagram.png",
                ...
                if (section.getUseCaseDiagramImage() != null) {
                    try (InputStream is = new ByteArrayInputStream(section.getUseCaseDiagramImage())) {
                        XWPFParagraph content = docx.createParagraph();
                        content.setIndentationFirstLine(600); // 0.5 inch indent
                        XWPFRun contentRun = content.createRun();
                        contentRun.setText("Use Case Diagram:");
                        contentRun.addBreak();
                        contentRun.addPicture(is,
                                Document.PICTURE_TYPE_PNG,
                ...
                if (section.getSequenceDiagramImage() != null) {
                    try (InputStream is = new ByteArrayInputStream(section.getSequenceDiagramImage())) {
                        XWPFParagraph content = docx.createParagraph();
                        content.setIndentationFirstLine(600); // 0.5 inch indent
                        XWPFRun contentRun = content.createRun();
                        contentRun.setText("Sequence Diagram:");
                        contentRun.addBreak();
                        contentRun.addPicture(is,
                                Document.PICTURE_TYPE_PNG,
